// Mighty battle is coming.In the stormy nether realms, demons are fighting against each other for supremacy
...
//     (e.g. + 10 is 10 and - 10 is - 10).However, there are some symbols('*' and '/') that can further alter the base damage by
// multiplying or dividing it by 2(e.g.in the name "m15*/c-5.0", the base damage is 15 + (-5.0) = 10 and then you need to
//   multiply it by 2(e.g. 10 * 2 = 20) and then divide it by 2(e.g. 20 / 2 = 10)).
...
function netherRealms(input) {
    ...
    for (const demon of demonsArr) {
        ...
        let damage = 0;
        let name = demon;
        if (demon.match(healthRegex)) {
            let match = demon.match(healthRegex);
        ...
        let substractOrMultiply = demon.match(dmgPattern);

        if (substractOrMultiply !== null) {
            for (const operand of substractOrMultiply) {
                if (operand === "*") {
                    damage *= 2;
                } else {
                    damage /= 2;
    ...
    // Without the object we created and the sort method we can get only 10pts.
