...
// which indicates how much spice can be mined on the first day. After it has been mined for a day, 
// the yield drops by 10, meaning on the second day it’ll produce 10 less spice than on the first, 
...
function solve(spice){
    let day = 0;
    let workersExpence = 26;
    let extractedSpice = spice;
    let sum = 0;
    while(spice >= 100){
        day++;
        sum = sum + (extractedSpice - workersExpence);
